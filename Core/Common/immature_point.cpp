...
#include "Common/residual_projections.h"
#include "Utils/global_funcs.h"

namespace ds_slam
{

ImmaturePoint::ImmaturePoint(int u_, int v_, FrameHessian *host_, float type, int isCorner_, CalibHessian *HCalib)
    : u(u_), v(v_), host(host_), my_type(type), isCorner(isCorner_), idepth_min(0), idepth_max(NAN), lastTraceStatus(IPS_UNINITIALIZED)
{
                                        ...
                                        float idepth)
{
    ...
    for (int idx = 0; idx < patternNum; idx++)
    {
        ...
        float d_idepth = DeriveIdepth(PRE_tTll, u, v, dx, dy, dxInterp, dyInterp, drescale);
