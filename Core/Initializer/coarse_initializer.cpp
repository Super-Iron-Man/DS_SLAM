...
#include "Common/pixel_selector.h"
#include "Common/pixel_selector.hpp"
#include "Utils/nano_flann.hpp"
...
bool CoarseInitializer::TrackFrame(FrameHessian *newFrameHessian, std::vector<visualizer::Visualizer3D *> &wraps)
{
...
void CoarseInitializer::DebugPlot(int lvl, std::vector<visualizer::Visualizer3D *> &wraps)
{
    bool needCall = false;
    for (visualizer::Visualizer3D *ow : wraps)
    ...
    //visualizer::DisplayImage("idepth-R", &iRImg, false);
    for (visualizer::Visualizer3D *ow : wraps)
...
void CoarseInitializer::SetFirst(CalibHessian *HCalib, FrameHessian *newFrameHessian)
{
    ...
    int *statusMap = new int[w[0] * h[0]];
    int *iscornerMap = new int[w[0] * h[0]];
    ...
    for (int lvl = 0; lvl < pyrLevelsUsed; lvl++)
    {
        ...
        int npts;
        if (lvl == 0)
            npts = sel.MakeMaps(firstFrame, statusMap, iscornerMap, densities[lvl] * w[0] * h[0], 1, false, 2);
        else
