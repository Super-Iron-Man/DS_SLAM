...
void CoarseTracker::CalcGSSSE(int lvl, Mat88 &H_out, Vec8 &b_out, const SE3 &refToNew, AffLight AffG2L)
{
    acc.Initialize();
    ...
    __m128 a = _mm_set1_ps((float)(AffLight::FromToVecExposure(lastRef->ab_exposure, newFrame->ab_exposure, lastRef_aff_g2l, AffG2L)[0]));
...
Vec6 CoarseTracker::CalcRes(int lvl, const SE3 &refToNew, AffLight AffG2L, float cutoffTH)
{
    float E = 0;
    int numTermsInE = 0;
    ...
    Vec2f affLL = AffLight::FromToVecExposure(lastRef->ab_exposure, newFrame->ab_exposure, lastRef_aff_g2l, AffG2L).cast<float>();
    ...
    if (DebugPlot)
    {
        visualizer::DisplayImage("RES", resImage, false);
        visualizer::WaitKey(0);
        delete resImage;
    }
...
void CoarseTracker::SetCoarseTrackingRef(std::vector<FrameHessian *> frameHessians)
{
    ...
    MakeCoarseDepthL0(frameHessians); // create idepth map

    refFrameID = lastRef->shell->id;
    lastRef_aff_g2l = lastRef->AffG2L();
...
void CoarseTracker::DebugPlotIDepthMap(float *minID_pt, float *maxID_pt, std::vector<visualizer::Visualizer3D *> &wraps)
{
    if (w[1] == 0)
        return;
    ...
    int lvl = 0;
    {
        ...
        //visualizer::DisplayImage("coarseDepth LVL0", &mf, false);
        ...
        if (debugSaveImages)
        {
            ...
            visualizer::WriteImage(buf, &mf);
        }
    }
}

void CoarseTracker::DebugPlotIDepthMapFloat(std::vector<visualizer::Visualizer3D *> &wraps)
{
    if (w[1] == 0)
        return;
    int lvl = 0;
