#include "visualizer_pangolin.hpp"
...
namespace visualizer
{
    ...
    ViewerPangolin::ViewerPangolin(int w, int h, bool startRunThread)
    {
        this->w = w;
        this->h = h;
        running = true;
        ...
        if (startRunThread)
            runThread = boost::thread(&ViewerPangolin::Run, this);
    }
    ...
    void ViewerPangolin::Close()
    {
        running = false;
    }

    void ViewerPangolin::Join()
    {
        runThread.join();
    ...
    void ViewerPangolin::ResetInternal()
    {
        model3DMutex.lock();
    ...
    void ViewerPangolin::DrawConstraints()
    {
    ...
    void ViewerPangolin::PublishGraph(const std::map<uint64_t,
                                                     Eigen::Vector2i,
                                                     std::less<uint64_t>,
    ...
    void ViewerPangolin::PublishKeyframes(
    ...
    void ViewerPangolin::PublishCamPose(Frame *frame,
                                        CalibHessian *HCalib)
    {
        if (!setting_render_display3D)
            return;
    ...
    void ViewerPangolin::PushLiveFrame(FrameHessian *image)
    {
        if (!setting_render_displayVideo)
            return;
        if (disableAllDisplay)
    ...
    bool ViewerPangolin::NeedPushDepthImage()
    {
        return setting_render_displayDepth;
    }

    void ViewerPangolin::PushDepthImage(MinimalImageB3 *image)
    {
