...
void FullSystem::DebugPlotTracking()
{
    ...
    for (FrameHessian *f : frameHessians)
    {
        ...
        for (FrameHessian *f2 : frameHessians)
        {
            ...
            Vec2 affL = AffLight::FromToVecExposure(f2->ab_exposure, f->ab_exposure, f2->AffG2L(), f->AffG2L());
        ...
        visualizer::DisplayImageStitch(buf, images);
        idx++;
    }
...
void FullSystem::DebugPlot(std::string name)
{
    ...
    visualizer::DisplayImageStitch(name.c_str(), images);
    visualizer::WaitKey(5);
