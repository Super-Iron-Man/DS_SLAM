...
void FullSystem::FlagFramesForMarginalization(FrameHessian *newFH)
{
    ...
    for (int i = 0; i < (int)frameHessians.size(); i++)
    {
        ...
        Vec2 refToFh = AffLight::FromToVecExposure(frameHessians.back()->ab_exposure, fh->ab_exposure,
                                                   frameHessians.back()->AffG2L(), fh->AffG2L());
...
void FullSystem::MarginalizeFrame(FrameHessian *frame)
{
    ...
    // delete marginalized frame and update ids in window
    // note: !!!
    // delete all points when release frame, delete all residuals when release point,
    // so linearized residual not exist in system actually.
