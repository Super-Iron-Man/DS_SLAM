...
PhotometricUndistorter::PhotometricUndistorter(std::string gammaFile,
                                               ...
                                               int height)
{
    ...
    if (gammaFile == "" || vignetteImage == "")
    {
        printf("NO PHOTOMETRIC Calibration!\n");
    }

    // read G.
    std::ifstream f(gammaFile.c_str());
    printf("Reading Photometric Calibration from gammaFile %s\n", gammaFile.c_str());
    if (!f.good())
    {
        printf("PhotometricUndistorter: Could not open gammaFile!\n");
        return;
    }
    ...
    MinimalImage<unsigned short> *vm16 = visualizer::ReadImageBW_16U(vignetteImage.c_str());
    MinimalImageB *vm8 = visualizer::ReadImageBW_8U(vignetteImage.c_str());
...
Undistort *Undistort::GetUndistorterForFile(std::string calibFilename, std::string gammaFilename, std::string vignetteFilename)
{
    printf("Reading Calibration from file %s", calibFilename.c_str());

    std::ifstream f(calibFilename.c_str());
    if (!f.good())
    {
        f.close();
                    ...
                    &ic[4], &ic[5], &ic[6], &ic[7]) == 8)
    {
        ...
        u = new UndistortRadTan(calibFilename.c_str(), true);
        if (!u->IsValid())
        {
            delete u;
            return 0;
                         ...
                         &ic[0], &ic[1], &ic[2], &ic[3], &ic[4]) == 5)
    {
        if (ic[4] == 0)
        {
            ...
            u = new UndistortPinhole(calibFilename.c_str(), true);
            if (!u->IsValid())
            {
                delete u;
                return 0;
        ...
        else
        {
            ...
            u = new UndistortFOV(calibFilename.c_str(), true);
            if (!u->IsValid())
            {
                delete u;
                return 0;
    ...
    // clean model selection implementation.
                         ...
                         &ic[4], &ic[5], &ic[6], &ic[7]) == 8)
    {
        u = new UndistortKB(calibFilename.c_str(), false);
        if (!u->IsValid())
        {
            delete u;
            return 0;
                         ...
                         &ic[4], &ic[5], &ic[6], &ic[7]) == 8)
    {
        u = new UndistortRadTan(calibFilename.c_str(), false);
        if (!u->IsValid())
        {
            delete u;
            return 0;
                         ...
                         &ic[4], &ic[5], &ic[6], &ic[7]) == 8)
    {
        u = new UndistortEquidistant(calibFilename.c_str(), false);
        if (!u->IsValid())
        {
            delete u;
            return 0;
                         ...
                         &ic[4]) == 5)
    {
        u = new UndistortFOV(calibFilename.c_str(), false);
        if (!u->IsValid())
        {
            delete u;
            return 0;
                         ...
                         &ic[4]) == 5)
    {
        u = new UndistortPinhole(calibFilename.c_str(), false);
        if (!u->IsValid())
        {
            delete u;
            return 0;
...
void Undistort::LoadPhotometricCalibration(std::string gammaFile, std::string noiseImage, std::string vignetteImage)
{
    photometricUndist = new PhotometricUndistorter(gammaFile, noiseImage, vignetteImage, GetOriginalSize()[0], GetOriginalSize()[1]);
}

template <typename T>
ImageAndExposure *Undistort::UndistortImage(const MinimalImage<T> *image_raw, float exposure, double timestamp, float factor) const
{
...
template ImageAndExposure *Undistort::UndistortImage<unsigned char>(const MinimalImage<unsigned char> *image_raw, float exposure, double timestamp, float factor) const;
template ImageAndExposure *Undistort::UndistortImage<unsigned short>(const MinimalImage<unsigned short> *image_raw, float exposure, double timestamp, float factor) const;
